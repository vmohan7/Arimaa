...
public class Utilities {
	 ...
	 */
	public static byte[] getFeatures(ArimaaState state, ArimaaMove move){
		byte[] features = new byte[FeatureConstants.NUM_FEATURES + ADDITIONAL_FEATURES];
		GameState currState = new GameState();
		currState.playFullClear(move, state.getCurr());
		
		FeatureExtractor fe = new FeatureExtractor(state.getPrev(), state.getCurr());
	 ...
	 */
	public static double logLinearEvaluation(ArimaaState state, ArimaaMove move, byte[] weights){
	...
	public static void TDUpdate(ArimaaState state, ArimaaState nextState, int reward, double eta, byte[] weights){
